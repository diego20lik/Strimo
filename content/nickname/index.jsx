...
function CambiarRole() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true)
    await usuarios.where("nickname", "==", nombreUsuarioRef.current.value).get().then(snap=>{
        if(snap.empty){
            usuarios.where("user", "==", currentUser.email).get().then(snapshot=>{
    ...
    <>
      ...
      <div className="degradado fondo">
        <form className="form-perfil" onSubmit={handleSubmit}>
          ...
          <button className="primario">Actualizar Perfíl</button>
        </form>
      </div>
    </>
  );
