...
function Conciertos({ data }) {
  ...
  const checkScroll = () => {
    setPosition({scroll: ref.current.scrollLeft, width: ref.current.clientWidth})
      ...
      <div className="section">
        ...
        <div className="prueba">
          ...
          <div className="carousel" ref={ref}>
            <InfiniteCarousel data={data} position={position} left={left} right={right} checkScroll={checkScroll}/>
...
const ElementPosition = ({element, position})=>{
    ...
    return <div style={{transition: "all 1s"}}>
      {element}
    </div>
}
...
const InfiniteCarousel = ({data, position, left, right, checkScroll})=>{
  ...
  return ( 
    <>
      {arr.map(video=>{
        return <ElementPosition element={video} position={position}/>
      })}
    </>
  )
}