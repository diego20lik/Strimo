...
function Index() {
  ...
  useEffect(() => {
    const unsubscriber = firebase.firestore().collection("Videos").doc(id).get().then(snap=>{
      ...
      firebase.firestore().collection("Pagos").where("usuario", "array-contains", currentUser.uid).where("video", "==", id).get().then(snap=>{
        if(snap.empty){
          history.push(`/pago/${id}`)
          return
        }
        firebase.firestore().collection("Videos").doc(id).get().then(snap=>{
          setInfo(snap.data())
        })
        setShow(true)
        snap.forEach(doc=>{
  ...
  return (
    ...
    {show&&
      ...
      <div className="player-fondo">
        <Video titulo={info.titulo} url={link}/>
        {tipo=="VIP"&&
        <div className="chat">
