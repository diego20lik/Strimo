...
function Index() {
  ...
  const handleFireBaseUpload = async (e) => {
    ...
    setLoading(true)
    if (videoAsFile === "") {
      setErr(`El archivo seleccionado no es una imagen, es: ${typeof videoAsFile}`);
      setLoading(false)
    }
    const uploadTask = storage
        .ref(`/users/${currentUser.uid}/videos/v${currentUser.uid}${time.now().toMillis()}`)
        .put(videoAsFile);
        ...
        uploadTask.then(async (doc) => {
            ...
            .ref(`/users/${currentUser.uid}/videos/i${currentUser.uid}${time.now().toMillis()}`)
            .put(imageAsFile)
    ...
    <>
      ...
      <div className="degradado fondo">
        <form className="form-perfil" onSubmit={handleFireBaseUpload}>
          <h1>Carga un nuevo video para tu espacio</h1>
          ...
          <div className="file-form">
            <h4>Coloca una Imagen para el Video</h4>
            {imageAsUrl.imgUrl == "" ? (
          ...
          <button className="primario">Subir Video</button>
        </form>
      </div>
    </>
  );
